  ...
  return (
    <ChartContext.Provider value={{ config }}>
      <div
        ...
        ref={ref}
        className={cn(
          "flex aspect-video justify-center text-xs [&_.recharts-cartesian-axis-tick_text]='#ccc']]='#fff']]='#ccc']]='#ccc']]='#fff']],
          className,
        )}
        {...props}
      >
...
const ChartStyle = ({ id, config }; config) => {
          ...
          {payload.map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={cn(
                  "flex w-full flex-wrap items-stretch gap-2 [&>svg],
                  ...
                  formatter(item.value, item.name, item, index, item.payload)
                    ...
                    >
                      <div className="grid gap-1.5">
                        {nestLabel ? tooltipLabel="text-muted-foreground">{itemConfig?.label || item.name}</span>
                      </div>
                      {item.value && (
    ...
    >
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn("flex items-center gap-1.5 [&>svg])}
          >
